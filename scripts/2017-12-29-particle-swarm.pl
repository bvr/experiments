...
use List::AllUtils qw(sum all uniq);
use Data::Dump qw(dd pp);

my $particles = load_particles(\*DATA);
is move_particles(particle => $particles)->{closest}{id}, 243, 'Part 1 - closest particle';
is move_particles(particle => $particles, remove_collisions => 1)->{particles_remaining}, 648, 'Part 2 - remaining after collisions';

done_testing;

fun load_particles($fh) {
    my @particle;
    my $id = 0;
    while(<$fh>) {
        if(/p=<(.*?)>, v=<(.*?)>, a=<(.*?)>/) {
...
fun move_particles(:$particle, :$remove_collisions = 0) {
    my @particle = eval pp @$particle;
    my $last = '';
    while(1) {
        for my $p (@particle) {
            next unless defined $p;
            $p->{vel}[$_] += $p->{acc}[$_] for 0..2;
            $p->{pos}[$_] += $p->{vel}[$_] for 0..2;
            $p->{dist} = sum(map { abs($_) } @{ $p->{pos} });
        }
        my @closest = sort { $a->{dist} <=> $b->{dist} } @particle;
        if($remove_collisions) {
            my @remove_ids = ();
            for my $i (0 .. $#closest-1) {
                if(all { $closest[$i]{pos}[$_] == $closest[$i+1]{pos}[$_] } 0..2) {
                    push @remove_ids, $closest[$i]{id}, $closest[$i +1]{id};
                }
            }
            for my $id (uniq @remove_ids) {
                $particle[$id] = undef;
            }
        }

        # dd [ map { $_->{id} } @closest[0..10] ];
        my $sample = pp([ map { $_->{id} } @closest ]);
        if($sample eq $last) {
            return { closest => $closest[0], particles_remaining => scalar grep { defined && defined $_->{id} } @particle };
        }
        else {
            $last = $sample;
        }
