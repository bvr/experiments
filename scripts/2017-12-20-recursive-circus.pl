...
package Program {
    ...
    has children => (is => 'rw', isa => ArrayRef [InstanceOf ['Program']], predicate => 1);

    method weight_with_children {
        return sum(0, map { $_->weight_with_children } @{$self->children}) + $self->weight;
    }

    method is_balanced {
        return 1 unless @{ $self->children };
        my $weight_of_first = $self->children->[0]->weight_with_children;
        return 1 if all { $_->weight_with_children == $weight_of_first } @{ $self->children };
        return 0;
    }
}
...
my %children;
while(<DATA>) {
    if(my ($name, $weight, $above) = /^(\w+) \((\d+)\)(?:\s*->\s*([\w, ]+))?/) {
        push @programs, Program->new(name => $name, weight => $weight);
        my @above = defined $above ? split(/,\s*/, $above) : ();
        $children{$name} = \@above;
    }
}

my %programs_by_name = map { $_->name => $_ } @programs;
my %not_root = ();
for my $parent (sort keys %children) {
    $programs_by_name{$parent}->children([ map { $programs_by_name{$_} } @{$children{$parent}} ]);
    @not_root{ @{$children{$parent}} } = ();
}

my (@root) = grep { ! exists $not_root{$_->name} } @programs;
is @root, 1, 'Only one root';
is $root[0]->name, 'bsfpjtc', 'Correct name of the root';

my $min_balance;
for my $p (@programs) {
    if(! $p->is_balanced) {
        if(! defined $min_balance || $min_balance->weight_with_children > $p->weight_with_children) {
            $min_balance = $p;
        }
    }
}

# find which is different and by how much.  Then answer is item's weight minus the difference
my %weights = ();
for my $ch (@{ $min_balance->children }) {
    my $ch_weight_with_children = $ch->weight_with_children;
    $weights{ $ch_weight_with_children } ||= [];
    push @{ $weights{ $ch_weight_with_children } }, $ch;
}

# assumes we have only one bad weight
my ($bad_weight)  = grep { @{ $weights{$_} } == 1 } keys %weights;
my ($good_weight) = grep { $_ != $bad_weight } keys %weights;

my $difference = $bad_weight - $good_weight;
is $weights{$bad_weight}->[0]->weight - $difference, 529, 'Correct weight - the answer part 2';
is $weights{$bad_weight}->[0]->name, 'utnrb', 'Name of the node with incorrect weight';
