...
my $input_code = list(imap { chomp; $_ } iter(\*DATA));
is count_steps(code => [0, 3, 0, 1, -3], start => 0), 5,      'Assignment example';
is count_steps(code => $input_code,      start => 0), 364539, 'My input';

is count_steps(code => [0, 3, 0, 1, -3], start => 0, reduce => 1), 10,       'Assignment example - part 2';
is count_steps(code => $input_code,      start => 0, reduce => 1), 27477714, 'My input - part 2';
...
fun count_steps(:$code, :$start = 0, :$reduce = 0) {
    ...
    while($pc >= 0 && $pc <= $#code) {
        ...
        # jump and increment current jump instruction
        my $cur_loc = $pc;
        my $offset = $code[$pc];
        $pc += $offset;
        if($reduce && $offset >= 3) {
        ...
        $steps++;
        # show_status(step => $steps, code => \@code, pc => $pc);
    }
    return $steps;
}
