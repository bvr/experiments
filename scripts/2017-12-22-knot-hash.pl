...
my $hashed = knot_hash(list => [0, 1, 2, 3, 4], lengths => [ 3, 4, 1, 5 ]);
is $hashed->[0] * $hashed->[1], 12, 'Example';

my $input = trim(scalar <DATA>);
my $lengths = [ split /,/, $input ];
my $standard = knot_hash(list => [0..255], lengths => $lengths);
is $standard->[0] * $standard->[1], 212, 'Input';

is_deeply lengths_from_string('1,2,3'), [49,44,50,44,51,17,31,73,47,23], 'Lengths from string transformation';
...
my %tests = (
    ...
    $input     => '96de9657665675b51cd03f0b3528ba26',
);

for my $str (sort keys %tests) {
    my $hash = dense_hash(knot_hash(list => [0 .. 255], lengths => lengths_from_string($str), rounds => 64));
    is $hash, $tests{$str}, "Knot hash for '$str'";
}

done_testing;

fun knot_hash(:$list, :$lengths, :$rounds = 1) {
    my $cp = 0;
    my $skip = 0;
...
=item * Reverse the order of that length of elements in the list, starting with the element at the current position.

=item * Move the current position forward by that length plus the skip size.

=item * Increase the skip size by one.
...
=item * The list begins as [0] 1 2 3 4 (where square brackets indicate the current position).

=item * The first length, 3, selects ([0] 1 2) 3 4 (where parentheses indicate the sublist to be reversed).

=item * After reversing that section (0 1 2 into 2 1 0), we get ([2] 1 0) 3 4.

=item * Then, the current position moves forward by the length, 3, plus the skip size, 0: 2 1 0 [3] 4. Finally, the skip size increases to 1.

=item * The second length, 4, selects a section which wraps: 2 1) 0 ([3] 4.

=item * The sublist 3 4 2 1 is reversed to form 1 2 4 3: 4 3) 0 ([1] 2.

=item * The current position moves forward by the length plus the skip size, a total of 5, causing it not to move because it wraps around: 4 3 0 [1] 2. The skip size increases to 2.

=item * The third length, 1, selects a sublist of a single element, and so reversing it has no effect.

=item * The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size increases to 3.

=item * The fourth length, 5, selects every element starting with the second: 4) ([3] 0 1 2. Reversing this sublist (3 0 1 2 4 into 4 2 1 0 3) produces: 3) ([4] 2 1 0.

=item * Finally, the current position moves forward by 8: 3 4 2 1 [0]. The skip size increases to 4.
