...
{
    my @max = max_register_value($example_in);
    is $max[0], 1,  'Example';
    is $max[1], 10, 'Example - maximum ever part 2';
}

{
    my @max = max_register_value(\*DATA);
    is $max[0], 5966, 'Part 1';
    is $max[1], 6347, 'Part 2';
}

done_testing;

=head2 max_register_value

    my ($max_at_end, $max_seen) = max_register_value($filehandle);

Runs the instructions per the assignment, returns highest value of all registers
at the end of processing and also highest value seen along the process.

=cut

sub max_register_value {
    my $fh = shift;
    my %registers;
    my $max_ever;
    while(<$fh>) {
        if(my ($reg, $op, $num, $cond_reg, $cond_op, $cond_num) = /^(\w+) (inc|dec) ([+-]?\d+) if (\w+) (<|>|<=|>=|==|!=) ([+-]?\d+)/) {
            if(eval "\$registers{\$cond_reg} $cond_op $cond_num") {
                my $perl_op = $op eq 'inc' ? "+="
                            : $op eq 'dec' ? "-="
                            : die "unknown operator $op";

                eval "\$registers{\$reg} $perl_op $num";
                my $max_of_all = max(values %registers);
                $max_ever = $max_of_all
                    if ! defined $max_ever || $max_of_all > $max_ever;
            }
        }
    }
    return max(values %registers), $max_ever;
}
